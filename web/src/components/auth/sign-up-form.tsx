...
export function SignUpForm() {
  ...
  const handleGoogleSignUp = async () => {
    ...
    try {
      ...
      setError("Google Sign Up is not configured yet. Please use email registration.");
    }
  };
  ...
  return (
    ...
    >
      ...
      <p className="text-sm text-[var(--muted-foreground)] mb-6">
        Start your personalized Socratic learning journey today
      </p>
              ...
              return (
                <div
                  ...
                  className={`flex items-center gap-2 text-xs transition-colors ${
                    met ? "text-emerald-400" : "text-[var(--muted-foreground)]"
                  }`}
                >
                  <CheckCircle2
                    className={`w-3.5 h-3.5 ${
                      met ? "text-emerald-400" : "opacity-40"
                    }`}
                  />
                  <span>{req.label}</span>
                </div>
